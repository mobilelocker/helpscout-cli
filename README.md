...
For long HTML/text bodies, prefer a file over inline text: MCP `filePath` (absolute path) or CLI `--file <path>`. Provide either the inline field (`text`/`body`/`--text`/`--body`) or `filePath`/`--file`, not both. Applies to Docs article create/update/draft and Inbox reply/note/conversation create. Article create/update still use the JSON APIs (unlike multipart `upload_article`).
...
# Create / update / draft body from a file (alternative to --text)
helpscout docs article create --collection <collection-id> --name "Guide" --file ./body.html
...
**MCP (`create_article` / `update_article` / `save_article_draft`):** pass `categories` as a string array, e.g. `"categories": ["<category-id>"]`. On update, omit the field to leave categories unchanged; use `"categories": null` or `"clearCategories": true` for Uncategorized. For body HTML, pass either `text` or `filePath` (absolute path on disk). Same `filePath` pattern on inbox `reply_to_conversation`, `add_note`, and `create_conversation` (`body` or `filePath`).
...
```sh
npm test              # unit tests (offline; no live Help Scout calls)
npm run test:integration  # live API tests (requires HELPSCOUT_* credentials)
